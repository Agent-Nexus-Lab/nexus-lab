...
from experiments.runtime.runtime import plan_day as plan_day_funct
from experiments.runtime.runtime import parse_now, DEFAULT_TIMEZONE
...
def plan_day(req: PlanDayRequest, db: Session = Depends(get_db)):
    ...
    print("数据库读到的 events 数量：", db.query(Event).count())
    events = []
    ...
    try:
        result = plan_day_funct(
            ...
            # 方便调试这改成固定时间，记得再改回来
            now = datetime.fromisoformat("2026-05-10T08:00:00+08:00"),
            include_debug = True
        )
        data = result["data"]
