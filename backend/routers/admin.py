...
def list_sources(page: int = 1, page_size: int = 20, db: Session = Depends(get_db)):
    ...
    return {
        "code": 0,
        "data": SourceListData(items=items, total=total, page=page, page_size=page_size).model_dump(mode="json"),
        "message": "ok",
    }
...
def list_events(page: int = 1, page_size: int = 20, db: Session = Depends(get_db)):
    ...
    return {
        "code": 0,
        "data": EventListData(items=items, total=total, page=page, page_size=page_size).model_dump(mode="json"),
        "message": "ok",
    }
