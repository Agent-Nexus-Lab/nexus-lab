...
├── eval.json               # 评测集（15 条典型 query，含第二阶段反馈/排除语义）
├── test_intent_parser.py   # 回归测试（25 个用例）
...
| 换一个/换点/换个 | `(?:换一个\|换点\|换个)([^\s]*){0,8}` | "换一个" → `{field: "excluded_keywords", value: "_switch_request"}` |

### 5.7 软约束 (soft_constraints) — 偏好权重

| 模式 | 权重 | 示例 |
|---|---|---|
| 特别喜欢/非常喜欢/超爱/特别想/更偏 | `1.5` | "特别喜欢天文" → 权重 1.5 |
...
| eval_008 | 换一个，不要重复推荐刚才的活动 | hard_constraint 排除"重复推荐" + 切换语义 |
| eval_009 | 不想太累，轻松一点就好 | 风格"轻松" + hard_constraint "太累" |
| eval_010 | 更偏AI方面的活动，最好不要讲座 | soft_constraint weight≥1.5 + hard_constraint 排除 |
| eval_011 | 不要毕业季相关的活动 | hard_constraint 排除"毕业季" |
| eval_012 | 换点不一样的，上次的都不喜欢 | hard_constraint 排除 + 反馈语义 |
| eval_013 | 不要体育，想看轻松的工作坊 | 排除+偏好+风格 三合一 |
| eval_014 | 最近别推讲座了，想看AI或者创业 | 多兴趣 + 别推排除 |
| eval_015 | 想看新鲜的，避免重复推荐 | hard_constraint 排除"重复推荐" |
